 ...
 */
CREATE OR REPLACE FUNCTION retina.search_strict_fuzzy_tool (
...
AS $$
  BEGIN
    IF array_length(lexemes, 1) = 0 THEN
      RETURN QUERY
        SELECT * FROM retina.search_strict_tool (
    ...
    RETURN QUERY SELECT * FROM retina.search_fuzzy_ids_tool (
      lexemes,
      ARRAY (
        SELECT id FROM retina.search_strict_tool (
          organization_id := organization_id,
          owner_ids := owner_ids,
          brand_ids := brand_ids,
          type_ids := type_ids,
